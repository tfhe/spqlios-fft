...
void reim4_extract_2cols_from_reim_vector_setup(const uint64_t m, const uint64_t nrows, double*& dst, double**& src) {
  dst = new double[nrows * 16];
...
void reim4_extract_2cols_from_reim_vector_teardown(const uint64_t m, const uint64_t nrows, double*& dst, double**& src) {
  for (uint64_t i = 0; i < nrows; ++i) delete[] src[i];
  delete[] src;
  delete[] dst;
}
...
template <void (*fnc)(uint64_t m, uint64_t nrows, uint64_t col, double* dst, double** src)>
void benchmark_reim4_extract_2cols_from_reim_vector(benchmark::State& state) {
  const uint64_t m = state.range(0);
  const uint64_t nrows = state.range(1);
  const uint32_t col = rand() % ((m >> 2) - 1);

  double** src;
  double* dst;
  reim4_extract_2cols_from_reim_vector_setup(m, nrows, dst, src);

  for (auto _ : state) {
    fnc(m, nrows, col, dst, src);
  }

  reim4_extract_2cols_from_reim_vector_teardown(m, nrows, dst, src);
}
...
BENCHMARK(benchmark_reim4_extract_2cols_from_reim_vector<reim4_extract_2cols_from_reim_vector_ref>)->ARGS;
